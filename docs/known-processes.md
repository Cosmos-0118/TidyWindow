...
The Known Processes workspace helps administrators tame noisy background services and suspicious executables. It combines a curated catalog of Windows services, user-defined preferences, and a live Anti-System scanner that keeps watch for untrusted processes. The page is split into three pivots—Catalog, Settings, and Anti-System—so users can jump between recommendations, policy tuning, and runtime telemetry.

## Purpose

-   **Catalog Guidance**: Surface vetted recommendations for services that can be safely auto-stopped or should remain running.
...
-   **Threat Hunting**: Flag unsigned or anomalous processes via Anti-System scanning and provide tools to quarantine or whitelist them.
...
-   Quarantine actions in Anti-System prompt for acknowledgement before terminating processes.
...
-   Anti-System uses Defender APIs and local heuristics to classify suspicious processes into Critical/Elevated/Watch buckets.
...
-   All catalog updates, service actions, automation runs, imports/exports, and Anti-System verdicts are written to the Activity Log with contextual details.
...
└──────┬──────────────┬───────────────────────-┘
       ...
       ▼              ▼
┌────────────────--┐ ┌────────────────────────┐
│ProcessPreferences│ │AntiSystemViewModel.cs  │
│ViewModel.cs      │ │(Anti-System Scanner)   │
└──────┬───────────┘ └──────┬─────────────────┘
       │                    │
       ▼                    ▼
┌──────────────┐   ┌────────────────────────┐
│ProcessState  │   │AntiSystemScanService.cs│
...
-   **`KnownProcessesViewModel`** (`src/TidyWindow.App/ViewModels/KnownProcessesViewModel.cs`)
    ...
    -   Loads catalog entries, maps preferences, and tracks the active pivot (Catalog, Settings, Anti-System).
...
-   **`AntiSystemViewModel` + `AntiSystemScanService`**
...
-   **Category Accordion**: Displays catalog groups with risk badges and descriptions.
...
-   **Process Table**: Filterable list of catalog entries with columns for current action, source, last updated, and notes.
...
1. **Initialization**
    ...
    - Anti-System pivot reads persisted hits before the first manual scan.
...
-   **`AntiSystemHitViewModel`**: Represents suspicious processes with severity, last action message, and command handlers.
...
4. **Whitelist Intentionally**: Anti-System whitelists persist; document notes so future reviewers know why an entry is trusted.
...
4. **Extend Anti-System Carefully**: New heuristics should default to Watch level until validated; always log evidence strings.
...
-   **Dialogs**: Import/export and Anti-System holdings use WPF modal windows with owner linking for proper focus management.
-   **Telemetry**: Automation and Anti-System results feed PulseGuard toasts when noteworthy events occur (e.g., suspicious hits).
