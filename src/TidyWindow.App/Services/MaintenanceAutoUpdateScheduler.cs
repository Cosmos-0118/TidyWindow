...
public sealed class MaintenanceAutoUpdateScheduler : IDisposable
{
    ...
    private async Task<MaintenanceAutomationRunResult> RunOnceInternalAsync(bool isBackground, CancellationToken cancellationToken)
    {
        ...
        {
            return MaintenanceAutomationRunResult.Skipped(DateTimeOffset.UtcNow, "Automation disabled");
        }

        if (!_settings.UpdateAllPackages && (_settings.Targets.IsDefaultOrEmpty || _settings.Targets.Length == 0))
        {
            return MaintenanceAutomationRunResult.Skipped(DateTimeOffset.UtcNow, "No packages selected");
        }

        if (IsMaintenanceWorkActive())
        {
            return MaintenanceAutomationRunResult.Skipped(DateTimeOffset.UtcNow, "Maintenance busy");
        }

        var timeout = isBackground ? 0 : Timeout.Infinite;
        if (!await _runLock.WaitAsync(timeout, cancellationToken).ConfigureAwait(false))
        {
            return MaintenanceAutomationRunResult.Skipped(DateTimeOffset.UtcNow, "Another run is in progress");
        }
        ...
        try
        {
            ...
            try
            {
                snapshot = await _inventoryService.GetInventoryAsync(cancellationToken).ConfigureAwait(false);
            }
            catch (Exception ex)
            {
                _activityLog.LogError("Maintenance automation", $"Unable to load inventory: {ex.Message}");
                return MaintenanceAutomationRunResult.Skipped(DateTimeOffset.UtcNow, "Inventory failed");
            }
            ...
            if (plan.Count == 0)
            {
                ...
                var skippedResult = MaintenanceAutomationRunResult.Create(timestamp, Array.Empty<MaintenanceAutomationActionResult>());
                _activityLog.LogInformation("Maintenance automation", "Run completed — no updates available.");
            ...
            foreach (var entry in plan)
            {
                ...
                try
                {
                    ...
                    actions.Add(new MaintenanceAutomationActionResult(entry.Manager, entry.PackageId, entry.DisplayName, true, result.Success, summary));
                }
                catch (Exception ex)
                {
                    actions.Add(new MaintenanceAutomationActionResult(entry.Manager, entry.PackageId, entry.DisplayName, true, false, ex.Message));
                }
            }
    ...
    private IReadOnlyList<AutomationPlanEntry> BuildPlan(PackageInventorySnapshot snapshot)
    {
        ...
        var list = new List<AutomationPlanEntry>();
    ...
    private void LogRunResult(MaintenanceAutomationRunResult result)
    {
        ...
        {
            _activityLog.LogInformation("Maintenance automation", "Automation run skipped.");
            return;
        }
    ...
    private static IEnumerable<string> BuildActionDetails(IReadOnlyList<MaintenanceAutomationActionResult> actions)
    {
        ...
        {
            var status = action.Success ? "Success" : "Failed";
            var message = string.IsNullOrWhiteSpace(action.Message) ? status : action.Message.Trim();
            yield return $"{action.DisplayName}: {message}";
        }
    }
